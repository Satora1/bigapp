...
const BookCard = ({ id, title, genre, coverColor, coverUrl,coverUrl2, isLoanedBook = false }: Book) => {
    return (
        <li className={cn(isLoanedBook && "xs:w-52 w-full")}>
            <Link href={`/books/${id}`} className={cn(isLoanedBook && "w-full flex flex-col items-center")}>
                <BookCover coverColor={coverColor} coverImage={coverUrl} coverImage2={coverUrl2} />
